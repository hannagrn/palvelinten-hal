...
Tein microa varten kotihakemistoon kansion, latasin sinne micro-2.0.11-linux64-static.tar.gz-tiedoston (stabiili linux-versio) micron GitHubista ja purin tervapallon.
...
Operaatiosta muodostui uusin micro-2.0.11-hakemisto. Siirryin kansioon `cd micro-2.0.11/` ja siellä olikin vihreänä paistava binääri-micro. Tarkistin myös `ls -l micro` kyseisen ohjelman oikeudet. Ne olivat 755, eli rwx-rx-rx. Käyttäjällä on kaikki oikeudet, ryhmillä ja muilla on luku- ja suoritusoikeus. 
